...
export default function Page() {
  ...
  const handleSubmitFunction = async () => {
    ...
    try {
      const verificationCodeResponse = await axios.post("/api/users/verify", {
        username,
        verifyCode: finalOtp,
      });

      if (verificationCodeResponse.data.success === false) {
        console.log("Error verifying code");
        showToast(verificationCodeResponse.data.error);
      } else {
